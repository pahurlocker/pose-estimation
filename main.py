    ...
    while True:
        ...
        if count % 10 == 0:
            if len(scores) > people_counter:
                ...
                person = PersonDetection(session_name=session_name, event_name="appeared")
                db.session.add(person)
                db.session.commit()
            elif (len(scores) < people_counter) and (people_counter > 0):
                people_counter = len(scores)
                person = PersonDetection(session_name=session_name, event_name="disappeared")
                db.session.add(person)
                db.session.commit()
        count += 1
...
def detections():
    ...
    results = {
        "results": [{"id": p.id, "session_name": p.session_name, "event_name": p.event_name, "create_date_time": p.create_date_time} for p in detections.items],
        "pagination": {
